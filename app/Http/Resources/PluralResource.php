...
class PluralResource extends JsonResource
{
    public function toArray($request)
    {
        ...
        if ($request->isMethod('get') && $request->route('plural')) {
            return [
                'message' => __('plurals.show'),
                'data' => $data,
            ];
    ...
    private function successMessage($request): string
    {
        switch ($request->method()) {
            case 'POST':
                return __('plurals.created');
            case 'PATCH':
            case 'PUT':
                return __('plurals.updated');
            case 'GET':
                return __('plurals.list');
            case 'DELETE':
                return __('plurals.deleted');
            default:
                return __('plurals.list');
        }
    }
